...
error_reporting(1);
if (!isset($_SESSION['user_type']) || !in_array($_SESSION['user_type'], ['allied_dept_admin', 'allied_dept_checker', 'allied_dept_maker'])) {
...
if ($currentUserType === 'allied_dept_admin') {
    $createTypeName = 'Checker';
    $createTypeId   = 2;
} elseif ($currentUserType === 'allied_dept_checker') {
...
// allied_dept_checker should only see their own division users
...
if ($currentUserType === 'allied_dept_admin') {
    ...
    $sql = "SELECT * FROM allied_dept_users 
            WHERE type_id = 2
            ORDER BY division_name, sno DESC";
...
$authorities = [];
$resAuth = execute_query("SELECT id, department_name FROM allied_depts ORDER BY department_name ASC");
...
">
    ...
    <div class="card">
        ...
        <form method="POST" action="ajax/save_allied_dept_user.php">
            ...
            <div class="grid">
                ...
                <?php if ($currentUserType === 'allied_dept_checker'): ?>
                    <div class="form-group">
                        <label>District</label>

                        <select name="district_id[]" id="district_id" multiple required style="height:100px;">
                        </select>

                        <small style="color:#0f4c75;font-weight:600;">
    ...
    <div class="user-listings">
        <?php if ($currentUserType === 'allied_dept_admin'): ?>
    ...
    ORDER BY division_name ASC, sno DESC
";
                            ...
                            <table style="width:100%;border-collapse:collapse;font-size:14px;text-align:center;">
                                ...
                                <?php $i=1; foreach ($users as $u): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="ajax/delete_allied_dept_user.php?id=<?= $u['sno'] ?>&role=checker"
        ...
        FROM allied_dept_users m
        LEFT JOIN allied_dept_districts d 
            ON m.district_id = d.district_code
        WHERE m.type_id = 3
        AND m.creator_checker_id IN ($ids)
        ORDER BY d.district_name ASC, m.sno DESC
    ";
                            ...
                            <table style="width:100%;border-collapse:collapse;font-size:14px;text-align:center;">
                                ...
                                <?php $i=1; foreach ($users as $u): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="ajax/delete_allied_dept_user.php?id=<?= $u['sno'] ?>&role=maker"
...
";
                        ...
                        <table style="width:100%;border-collapse:collapse;font-size:14px; text-align:center;">
                            ...
                            <?php $i=1; foreach ($users as $u): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="ajax/delete_allied_dept_user.php?id=<?= $u['id'] ?>&role=maker"
                                           onclick="return confirm('Delete this maker?')"
                                           class="delete-btn">
                                            🗑 Remove
                                        </a>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (userType === "allied_dept_checker" && sessionDivisionId) {
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function loadDistricts(divId) {
                ...
                fetch("ajax/allied_dept_get_districts_for_maker.php?division_id=" + divId)
                    .then(res => res.json())
                    .then(data => {
            ...
            districtEl?.addEventListener("change", function () {
                ...
                Array.from(this.selectedOptions).forEach(opt => {
                    names.push(opt.getAttribute("data-name"));
                });

                districtNameEl.value = names.join(",");
            });

        });
    </script>

<?php if ($currentUserType === 'allied_dept_checker' || $currentUserType === 'allied_dept_admin'): ?>
    <script>
        document.addEventListener("DOMContentLoaded", function () {
