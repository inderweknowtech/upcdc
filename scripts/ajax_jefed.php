                ...
                if (isset($_POST['survey_id']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    if (isset($_POST['zone_name']) && is_array($_POST['zone_name'])) {
                        ...
                        foreach ($_POST['zone_name'] as $k => $v) {
                            ...
                            /* ===== KEEP OLD IMAGE ===== */
                            $zone_image_name = $_POST['existing_zone_image'][$k] ?? '';

                            /* ===== NEW IMAGE UPLOAD ===== */
                            if (!empty($_FILES['zone_image']['name'][$k])) {

                                $ext = pathinfo($_FILES['zone_image']['name'][$k], PATHINFO_EXTENSION);

                                $zone_image_name = time() . '_' . $survey_id . '_zone_' . $k . '.' . $ext;

                                move_uploaded_file(
                                    $_FILES['zone_image']['tmp_name'][$k],
                                    $upload_dir . $zone_image_name
                                );
                            }
                    ...
                    if (isset($_POST['prakhand_name']) && is_array($_POST['prakhand_name'])) {
                        ...
                        foreach ($_POST['prakhand_name'] as $k => $v) {
                            ...
                            /* ===== KEEP OLD IMAGE ===== */
                            $prakhand_image_name = $_POST['existing_prakhand_image'][$k] ?? '';

                            /* ===== NEW IMAGE UPLOAD ===== */
                            if (!empty($_FILES['prakhand_image']['name'][$k])) {

                                $ext = pathinfo($_FILES['prakhand_image']['name'][$k], PATHINFO_EXTENSION);

                                $prakhand_image_name = time() . '_' . $survey_id . '_prakhand_' . $k . '.' . $ext;

                                move_uploaded_file(
                                    $_FILES['prakhand_image']['tmp_name'][$k],
                                    $upload_dir . $prakhand_image_name
                                );
                            }
                    ...
                    /* ================= OTHER OFFICE INSERT ================= */
                    if (isset($_POST['other_office_name']) && is_array($_POST['other_office_name'])) {

                        foreach ($_POST['other_office_name'] as $k => $v) {

                            $name     = trim($_POST['other_office_name'][$k]);
                            $district = trim($_POST['other_office_district'][$k]);
                            $mobile   = trim($_POST['other_office_mobile'][$k]);
                            $email    = trim($_POST['other_office_email'][$k]);
                            $address  = trim($_POST['other_office_address'][$k]);

                            if ($name == '' && $district == '' && $mobile == '' && $email == '' && $address == '') {
                                continue;
                            }

                            /* ===== KEEP OLD IMAGE ===== */
                            $other_image_name = $_POST['existing_other_image'][$k] ?? '';

                            /* ===== NEW IMAGE UPLOAD ===== */
                            if (!empty($_FILES['other_office_image']['name'][$k])) {

                                $ext = pathinfo($_FILES['other_office_image']['name'][$k], PATHINFO_EXTENSION);

                                $other_image_name = time() . '_' . $survey_id . '_other_' . $k . '.' . $ext;

                                move_uploaded_file(
                                    $_FILES['other_office_image']['tmp_name'][$k],
                                    $upload_dir . $other_image_name
                                );
                            }
...
function upload_img($file, $society, $new_name, $maxDim = 1500)
{
    if (!isset($file) || $file['error'] !== UPLOAD_ERR_OK) {
        return ["error" => 0, "msg" => "Upload error", "file_name" => ""];
    }

    if (!is_uploaded_file($file['tmp_name'])) {
        return ["error" => 0, "msg" => "Invalid upload", "file_name" => ""];
    }

    $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));

    if (!in_array($ext, ['jpg', 'jpeg', 'png', 'gif'])) {
        return ["error" => 0, "msg" => "Invalid file type", "file_name" => ""];
    }

    if ($file['size'] > 50000000) {
        return ["error" => 0, "msg" => "File too large", "file_name" => ""];
    }

    $imgInfo = getimagesize($file['tmp_name']);
    if ($imgInfo === false) {
        return ["error" => 0, "msg" => "Invalid image", "file_name" => ""];
    }
