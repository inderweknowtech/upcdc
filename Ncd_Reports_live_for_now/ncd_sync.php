...
function sync_geo(mysqli $db): void
{
    ...
    if (!empty($data['result'])) {
        ...
        foreach ($data['result'] as $item) {
            ...
            if (!$r || empty($r['ward_code'])) {
                continue;
            }
            upsert($db, 'ncd_urban_local_body_wards', [
                'ward_code' => nvl_int($r['ward_code'])
            ], [
...
function sync_cooperatives(mysqli $db): void
{
    ...
    foreach ($data['result'] as $r) {
        ...
        upsert($db, 'ncd_cooperative_registrations', [
            'cooperative_id' => nvl_int($r['cooperative_id'] ?? null)
        ], [
            ...
            'sector_of_operation_type'    => nvl($r['sector_of_operation_type'] ?? null),
            'sector_of_operation'         => nvl($r['sector_of_operation'] ?? null),
            'functional_status'           => nvl($r['functional_status'] ?? null),
            'location_of_head_quarter'    => nvl($r['location_of_head_quarter'] ?? null),
            'full_address'                => nvl($r['full_address'] ?? null),
            'contact_person'              => nvl($r['contact_person'] ?? null),
            'designation'                 => nvl($r['designation'] ?? null),
            'email'                       => nvl($r['email'] ?? null),
            'category_audit'              => nvl($r['category_audit'] ?? null),
            'operation_area_location'     => nvl($r['operation_area_location'] ?? null),
            ...
            // ✅ FLAGS / SMALL INT (keep as int-safe)
